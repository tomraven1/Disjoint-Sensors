...
rad=centroids(1,:).^2+centroids(2,:).^2;

nanx = isnan(centroids(1,:));
centroids(:,nanx)= [];
sens(:,nanx)=[];

for kk=2:8
    
    MI(1,kk-1)=mi((centroids(1,:)'),(sens(kk,:)'));
...
for kk=1:length(C)
    ...
    MI(2,kk)=mi(centroids(1,:)',sens(C(kk,1),:)')+cmi(centroids(1,:)',sens(C(kk,2),:)',sens(C(kk,1),:)');
  
  %  MI2D(C(kk,1)-1,C(kk,2)-1)=mi(x(:,1),x(:,C(kk,1)))+cmi(x(:,1),x(:,C(kk,2)),x(:,C(kk,1)));
